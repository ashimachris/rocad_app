...
// Allow certain file formats
  ...
  if (isset($_FILES['attachement']) && $_FILES['attachement']['tmp_name'] != '') {
    ...
    if($uploaded_img){
        $sql="insert into `fixed_stock`(site,itemName,qty,amt,requi,assignto,info,preby,reference,suppl,supplphone,time_date)values('".mysqli_real_escape_string($config,$_POST["site"])."','".mysqli_real_escape_string($config,$_POST["itm"])."','".mysqli_real_escape_string($config,$_POST["qty"])."','".mysqli_real_escape_string($config,$_POST["amt"])."','".mysqli_real_escape_string($config,$_POST["req"])."','".mysqli_real_escape_string($config,$_POST["assignto"])."','".mysqli_real_escape_string($config,$_POST["info"])."','$preby','$tenDgt','".mysqli_real_escape_string($config,$_POST["supl"])."','".mysqli_real_escape_string($config,$_POST["supplphone"])."','$timeDate')";
      mysqli_query($config,$sql) or die(mysqli_error($config));

        // Insert image file name into database
        $supl = mysqli_real_escape_string($config,$_POST["supl"]);
        $insert = ("INSERT into invoices (file_name,reference,PlantNo,supl,ispaid,title,uploadby,lpo,uploaded_on) VALUES ('$fname','$tenDgt','N/A','$supl',0,'Fixed stock','$preby','$tenDgt','$timeDate')");
            ...
            if($insert){
                ...
                echo "<script>setTimeout(function(){window.location='fixed_stocks.php';},4200);</script>";
            }else{
  ...
  /*
    ...
    if(in_array($fileType, $allowTypes)){
        ...
        if(move_uploaded_file($_FILES["filen"]["tmp_name"], $targetFilePath)){
            // Insert image file name into database
$insert = ("INSERT into invoices (file_name,reference,PlantNo,supl,ispaid,title,uploadby,lpo,uploaded_on) VALUES ('".$NewName."','".$tenDgt."','N/A','".mysqli_real_escape_string($config,$_POST["supl"])."',0,'Fixed stock','".$preby."','".$tenDgt."',NOW())");
              mysqli_query($config,$insert) or die(mysqli_error($config));
            if($insert){
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header"> 
              <h3 class="box-title"><a href="#"><a href="fixed_stocks.php"><?php echo "Fixed Stock" ?></a></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <div class="form-group">
                  ...
                  <img src="pace/fixed.png" style="height:25%; padding-top:50px;" class="hidden-mobile">
 ...
 <li>
             ...
             <li>
              <label for="">Supplier Phone:</label>
               <input type="number"  min="0" name="supplphone" required class="form-control">
            </li>
             <li>
              <label for="">Item Name:</label>
               <input type="text" name="itm" required class="form-control">
            </li>
            <li>
             ...
             <li>
              <label for="">Amount:</label>
               <input type="number" min="0" name="amt" class="form-control">
            </li>
            <li>
            ...
            <li>
              <label for="">Assign to:</label>
               <select name="assignto" required class="form-control"><option value="" selected>::Select Staff::</option><?php while($row_asset=mysqli_fetch_assoc($asset)){?><option value="<?php echo $row_asset['id']; ?>"><?php echo $row_asset['fullname']; ?></option><?php }?></select>
            </li>
            <li>
              <label for="">Upload invoice (JPEG,PNG,GIF,PDF):</label>
