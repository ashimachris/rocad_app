...
if(isset($_POST["sbt"])){
 ...
 /////////////////////////////
      $reqfor=mysqli_real_escape_string($config,$_POST["reqfor"]);
      $qty=mysqli_real_escape_string($config,$_POST["qty"]);
      $PlantNo=mysqli_real_escape_string($config,$_POST["plantno"]);
      $qtyw=mysqli_real_escape_string($config,$_POST["qtyw"]);
      $infoD=mysqli_real_escape_string($config,$_POST["info"]);

$sql="UPDATE `storeloadingdetails` SET reqfor='$reqfor',qty='$qty',PlantNo='$PlantNo',qtyw='$qtyw',infoD='$infoD',modify_by='$preby' where id='$ids'";
  ...
  $msg="<font color='green'>Data successfully Updated.</font>";
   echo    "<script>setTimeout(function(){window.location='requi-notif.php?v=$reference';},2200);</script>";
  }
    }
?>
<style type="text/css">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header"> 
              <h3 class="box-title" ><a href="#"><a href="#">Requisition Update--L.P.O No.: (<?php echo $row_notii['reference']; ?>)</a></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
<ul>
  <li>            
              <label for="">Required for:</label>
               <textarea class="form-control" required name="reqfor"><?php echo $row_notii['reqfor']; ?></textarea>         
        </li></ul>
         
        <table class="table table-bordered txt" id="w" border="0" >
          <ul class="row'+i+'"><li><div class="form-group"><div class="form-wrapper"><label for="">Quantity:</label><input type="number" min="1" class="form-control" required name="qty" id="qty" value="<?php echo $row_notii['qty']; ?>"></div><div class="form-wrapper"><label for="">A/C Code:</label><select name="plantno" class="form-control" required><option value="" selected><?php echo $row_notii['PlantNo']; ?></option><option value="">N/A</option><?php while($row_asset=mysqli_fetch_assoc($asset)){?><option value="<?php echo $row_asset['sortno']; ?>"><?php echo $row_asset['sortno']; ?></option><?php }?></select></div></div></li><li><label for="">Quanity in Word:</label><input type="text" min="1" class="form-control" required name="qtyw" id="qtyw" value="<?php echo $row_notii['qtyw']; ?>"></li><li><label for="">Description:</label><textarea class="form-control" required name="info"><?php echo $row_notii['infoD']; ?></textarea></li><ul>
