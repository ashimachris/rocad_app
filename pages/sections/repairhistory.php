...
$asset = "SELECT * FROM `history` where assetID=$ids and title='Repair' order by id desc";
$assets = "SELECT * FROM `history` where assetID=$ids$mysql and title='Repair' order by id desc";
...
$sum = "SELECT sum(lprice) as allsum FROM `history` where title='Repair' and assetID=$ids$mysql";
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title" ><a href="repairhistory.php?v=<?php echo $ids; ?>"><?php echo "Repair History --- ".$plantNo; ?></a></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-hover" style=" text-transform: uppercase;">
                ...
                <?php $j=0;while($row_assets=mysqli_fetch_assoc($as_assets)){$j++; ?>
                  <tr style="cursor:pointer;<?php if($row_assets['isreturn']){ ?>text-decoration: overline;text-decoration-color: red;<?php }?>" onmouseover="$(this).css('background', '#d4edda');" onMouseOut="$(this).css('background', '#FFF');" <?php if($row_assets['isreturn']){ ?>title="Retured Item"<?php }?>>
                <td><?php echo $j; ?></td>
