...
if(isset($_GET['st'])and(!empty($_GET['st']))){
  ...
  $mysql="and (date(time_date) BETWEEN '$f' and '$t') $plants $st";
}
else{
$mysql="";
}
$qryasset = "SELECT * FROM assets WHERE status = 1 ORDER BY PlantNo";
$assetQry=mysqli_query($config,$qryasset) or die(mysqli_error($config));

$asset = "SELECT * FROM `history` WHERE `title` in('Aggregate','Sand','Laterite','Boulder','MC1','Aspalt','Blocks','S125','Reinforcement') order by id desc";
$assets = "SELECT * FROM `history` WHERE `title` in('Aggregate','Sand','Laterite','Boulder','MC1','Aspalt','Blocks','S125','Reinforcement') $mysql order by id desc";
...
$sum = "SELECT sum(CASE WHEN liter IS NULL THEN lprice ELSE lprice * liter end) as allsum FROM `history` WHERE `title` in('Aggregate','Sand','Laterite','Boulder','MC1','Aspalt','Blocks','S125','Reinforcement') $mysql";
...
<body class="hold-transition skin-blue sidebar-mini">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><a href="aggregate_report.php">AGGREGATE</a></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <p class="right"><span style="color:darkred; cursor: pointer;" id="myBtn">Filter By Date</p>
                <p class="center"><span style="color:#3c8dbc;"><b>Quantity In:</b></span><span style="color:darkred;">#<?php echo number_format($row_sum['allsum']);?><span style="color:#3c8dbc;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Quantity Out:</b></span><span style="color:darkred;">#<?php echo number_format(0,2);?><span style="color:#3c8dbc;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Balance:</b></span><span style="color:darkred;">#<?php echo number_format($row_sum['allsum']-0);?> </p>
                  </div>
              ...
              <table id="example1" class="table table-bordered table-hover" style=" text-transform: uppercase;">
                ...
                <?php $j=0;while($row_assets=mysqli_fetch_assoc($as_assets)){$j++; ?>
                  <tr style="cursor:pointer;background-color: #d4edda" onmouseover="$(this).css('background', '#FFF');" onMouseOut="$(this).css('background', '#d4edda');" onClick="window.location='aggregate_history.php?v=<?php echo $row_assets['PlantNo'];?>'">
                <td><?php echo $j; ?></td>
  ...
  <div class="modal-content">
    ...
    <center><form><table>
                ...
                <th>Plant No</th>
                <td>&nbsp;&nbsp;&nbsp;</td>
                <td><select name="p" class="form-control"><option value="" selected>::Select Plant No::</option><option value="">N/A</option><?php while($row_asset=mysqli_fetch_assoc($assetQry)){?><option><?php echo $row_asset['sortno']; ?></option><?php }?></select><input type="hidden" name="tlt" value="<?php echo $tlt; ?>"></td>                   
                  </tr>
