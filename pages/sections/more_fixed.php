...
if(isset($_GET['tlt'])and(!empty($_GET['tlt']))){
  $tlt=$_GET['tlt'];
  $tlts="and itemName like'%$tlt%'";
}
if(isset($_GET['st'])and(!empty($_GET['st']))){
  $sts=$_GET['st'];
  $st="and site='$sts'";
}
  $mysql="where (date(time_date) BETWEEN '$f' and '$t') $tlts $st";
}
else{
$mysql="";
}
...
$asset = "SELECT * FROM `fixed_stock` order by id desc";
$assets = "SELECT * FROM `fixed_stock` $mysql order by id desc";
...
$sum = "SELECT sum(amt) as allsum,sum(qty) as qty FROM `fixed_stock` $mysql";
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><?php if($_GET['tlt']){ ?><a href="#">More Fixed Stock</a><?php }?> - - <span class='dropbtn label label-success'><?php echo $_GET['tlt']; ?></span></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <p class="right"><span style="color:darkred; cursor: pointer;" id="myBtn">Filter By Date</p>
                <p class="center"><b><span style="color:#3c8dbc;">Quantity(<?php echo $tlt; ?>):</span> <span style="color:darkred;">#<?php echo number_format($row_sum['qty']);?></span></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><span style="color:#3c8dbc;">Total amount:</span> <span style="color:darkred;">&#8358;<?php echo number_format($row_sum['allsum'],2);?></span></p></b>
                  </div>
              ...
              <table id="example1" class="table table-bordered table-hover" style=" text-transform: uppercase;">
                ...
                <?php $j=0;while($row_assets=mysqli_fetch_assoc($as_assets)){$j++;$ttl=$row_assets['itemName']; ?>
                  <tr style="cursor:pointer" onmouseover="$(this).css('background', '#d4edda');" onMouseOut="$(this).css('background', '#FFF');" onClick="window.location='#'">
                <td><?php echo $j; ?></td>
    ...
    <center><form><table>
                ...
                <td><select name="st" class="form-control"><option value="" selected>::Select Site::</option><option value="">N/A</option><?php while($row_sites=mysqli_fetch_assoc($as_site)){?><option value="<?php echo $row_sites['id']; ?>"><?php echo $row_sites['site_state']."-".$row_sites['site_lga']."-".$row_sites['site_loc']; ?></option><?php }?></select></td>                   
                  ...
                  <tr>
                   ...
                   <tr>
                <th><input type="hidden" name="tlt" value="<?php echo $tlt;?>"></th>
                <td>&nbsp;&nbsp;&nbsp;</td>
