...
if(isset($_GET["w"])and (!empty($_GET["w"]))){
  $w=$_GET["w"];
  $wq="";
  $ttl="Receiving Report";
  $lk=true;
}
  else{
  $ttl="Requisition Report";
  $lk=false;
  $wq=" and dept!=2023";
}
...
$assets = "SELECT * FROM `storeloadingdetails` where dept is not null $wq $ref $mysql AND NOT status=1 order by id desc";
...
<button onclick="window.location='denied_invoices.php'" style="width: 100px; height: 30px; font-size: 12px; border-radius: 10px; margin: 10px; padding: 10px 20px; background-color: #3498db; color: #fff; border: none; float:right; display: flex; justify-content: center; align-items: center;"
                class="pending" > All Denied</button>  
<!--<button onclick="window.location='complete_invoice.php'" style="width: 100px; height: 30px; font-size: 12px; border-radius: 10px; margin: 10px; padding: 10px 20px; background-color: #3498db; color: #fff; border: none; float:right; display: flex; justify-content: center; align-items: center;"
                class="pending" > Completed</button>-->                            

            </div>

            <!-- /.box-header -->
             <h2 style="text-align: center" class="box-title"><font color="darkgreen"> <?php echo $title ?> REQUSTION REPORT</h2> 
                ...
                <td><?php $siteID=$row_assets['fromsite'];require '../layout/site.php'; echo $row_site['site_state']."-".$row_site['site_lga']."-".$row_site['site_loc']  ?></td>
                 ...
                 <td onmouseover="$(this).css('background', '#d4edda');" onMouseOut="$(this).css('background', '#FFF');" <?php if($lk){$link="receiving_report.php?v=";}else{$link="more_rr.php?v=";}?> onClick="var sts=<?php echo $row_assets['status']; ?>;if(sts==0){alert('L.P.O No.: '+<?php echo $row_assets['reference']; ?>+'\nStill Pending\nPlease wait for approval!');}else{window.location='<?php $id=$row_assets['reference'];echo $link.$id;?>'}"><?php echo $row_assets['reference']; ?></td>
                  ...
                  <?php 
                    ...
                    $fname = 'uploads/' . $id . '.' . 'jpg'; // Check file
                    $dir_path = $fname;
                    ...
                    // $fetch_details = mysqli_query($config,"SELECT * FROM `daily_expenses_reports` WHERE reference='$id'");
                  ?>
                  <?php if($file_exist){  ?>
                    <td>Completed</td>
                 <?php  } else{ ?>
               
                       <td onmouseover="$(this).css('cursor', 'pointer');"  <?php $invoice_link="receiving_invoice.php?v=" ?>
                   onclick="window.location='<?php echo $invoice_link.$id; ?>' ">Upload invoice
                 </td>
                 ...
                 <?php } ?>
                   ...
                   <?php switch($row_assets['status']){case 0:echo "<span class='label label-warning'>Pending</span>";break;case 1:echo "<span class='label label-danger'>Denied</span>";break;case 2:echo "<span class='label label-success'>Approved</span>";break;case 4:echo "<span class='label label-success'>Received</span>";break;case 5:echo "<span class='label label-success'>Completed</span>";break;default:echo "<span class='label label-danger'>Unknown</span>";} ?></td>
