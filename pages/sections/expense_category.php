    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        ...
        <div class="box">
          ...
          <button onclick="window.location='/rocad_admin/pages/sections/category_list.php'" style="width: 120px; height: 30px; font-size: 12px; border-radius: 10px; margin: 10px; padding: 10px 20px; background-color: #3498db; color: #fff; border: none; float:right; display: flex; justify-content: center; align-items: center;"
                class="pending">Category List</button> 
                ...
                <tbody>
                  <?php $j=0; while($row_assets=mysqli_fetch_assoc($assets)) { $j++; ?>
                    <tr style="text-transform: uppercase; color: darkred;">
                      ...
                      <td>
                        <?php
                        $category_id = $row_assets['expense_category_id'];
                        $sub_category_id = $row_assets['expense_sub_category_id'];

                        if(!empty($category_id)){
                            $qry_details = "SELECT * FROM `expenses_category` WHERE id=$category_id";
                            $details=mysqli_query($config,$qry_details) or die(mysqli_error($config));
                            $expense_category_name = mysqli_fetch_assoc($details)['name'];
                            echo "Category: $expense_category_name <br />";
                          }
                          if(!empty($sub_category_id)){
                            echo "============<br />";
                            $qry_details = "SELECT * FROM `expenses_sub_category` WHERE id=$sub_category_id";
                            $details=mysqli_query($config,$qry_details) or die(mysqli_error($config));
                            $sub_category_name = mysqli_fetch_assoc($details)['name'];
                            echo "Sub Category: $sub_category_name <br />";
                          }
                        ?>  
                      </td> 
                          ...
                          <div class="dropdown-content">
                            ...
                            <?php 
                              if ($row_assets['status'] = 2) {
                                echo "
                                  <a href='edit_category.php?edit_id={$row_assets['id']}'>Edit</a>
                                ";
                              } else {
                                echo "<h5>Approved</h5>";
                              }
