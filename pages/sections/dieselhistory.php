...
$dp=$row_group[EngineType_dies_petr];
}
else{
...
$asset = "SELECT * FROM `history` where assetID=$ids and title='$dp' order by id desc";
$assets = "SELECT * FROM `history` where assetID=$ids$mysql and title='$dp'  order by id desc";
...
$sum = "SELECT sum(CASE WHEN liter IS NULL THEN lprice ELSE lprice * liter end) as allsum FROM `history` where title='$dp' and assetID=$ids$mysql";
$sql_sum=mysqli_query($config,$sum)or die(mysqli_error($config));
$row_sum=mysqli_fetch_assoc($sql_sum);
//echo $row_sum['allsum'];
$sum2 = "SELECT sum(CASE WHEN liter IS NULL THEN lprice ELSE  liter end) as allCs FROM `history` where title='$dp' and assetID=$ids$mysql";
...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <div class="wrapper">
    ...
    <?php include_once "../layout/topmenu.php"; allow_access_all(1,1,0,1,1,0,$usergroup); ?>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"  ><a href="dieselhistory.php?v=<?php echo $ids; ?>"><?php echo "History --- ".$plantNo."--- ".$dp; ?></a></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
<p class="center"><?php if($row_sum2['allCs']){?><span style="color:#3c8dbc;"><b>Total Liters Consume:</b></span> <span style="color:darkred;"><?php echo number_format($row_sum2['allCs'])."--LTR";?></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php }?><b><span style="color:#3c8dbc;">Amount of <?php echo $dp; ?> Consume:</b></span> <span style="color:darkred;"><?php echo "&#8358;".number_format($row_sum['allsum']);?></p>
              ...
              <table id="example1" class="table table-bordered table-hover" style=" text-transform: uppercase;">
               ...
               <th>S/N</th>
                ...
                <?php $j=0;while($row_assets=mysqli_fetch_assoc($as_assets)){$j++; ?>
                  <tr style="cursor:pointer" onmouseover="$(this).css('background', '#d4edda');" onMouseOut="$(this).css('background', '#FFF');" onClick="window.location='#.php?v=<?php $id=$row_assets['id'];echo $id;?>'">
                <td><?php echo $j; ?></td>
                  <td><?php echo $row_assets['PlantNo']; ?></td>
                  <td><?php echo $row_assets['liter']."LTR"; ?></td>
                  <td><?php echo $row_assets['lprice']; ?></td>
                  <td><?php echo "&#8358;".number_format($row_assets['lprice']*$row_assets['liter']); ?></td>
                  <td><?php echo $row_assets['time_date']; ?></td>
                  <td><?php $prebyID=$row_assets['pre_by'];require '../layout/preby.php';echo $row_preby['fullname']; ?></td>
                   <td><?php echo $row_assets['loadcarry']; ?></td>
